...
%
% This converts between fault vectors and fault parameters.
%
% itype = 1     convert fault vectors to fault parameters
%  INPUT:
%    Xin        9 x n fault vectors [k1 ; d1 ; n1]
%  OUTPUT:
%    Xout       n x 3 set of fault parameters [kap1 del1 lam1]
%
% itype = 0     convert fault vectors to fault parameters
%  INPUT:
%    Xin        n x 3 set of fault parameters [kap1 del1 lam1]
%  OUTPUT:
%    Xout       9 x n fault vectors [k1 ; d1 ; n1]
%
% calls xxx
% called by faultpar2CMT.m, CMT2faultpar.m
%
...
if itype==1
    ...
    % dip and strike
    % dip: delta is [0, 90] because we "force" the normal to point up
    % strike: kappa is unrestricted, but we force it to [0, 360]
    % NOTE: instead of xyz2tp.m, we could use rotmat.m (see below)
    [n1th,n1ph] = xyz2tp(n1);
    del1 = n1th;
    ...
    for ii=1:n
        ...
        % note: acos operation to get lambda assumes UNIT vectors k and d
        lam1(ii) = sig1 * acos( dot1 );
    end
    ...
    if idisplay==1
        for ii = 1:n
            ...
            disp(sprintf('%6i%6.1f%6.1f%6.1f',ii,kap1(ii),del1(ii),lam1(ii)));
...
elseif itype==0
    ...
    % To get the strike vector, rotate (-1,0,0) through angle minus kap about the axis (0,0,1)
    % To get the normal vector, rotate (0,0,1) through angle delta about the strike vector.
    % To get the slip vector, rotate the strike vector through angle lambda about the normal vector.
    k1 = zeros(3,n);
    d1 = zeros(3,n);
    n1 = zeros(3,n);
    for ii=1:n
        % fault normal vector
        R = rotmat(-kap(ii),3) * rotmat(-del(ii),1);
        n1(:,ii) = R*upvec;
        
        % slip vector
        R = rotmat(-kap(ii),3) * rotmat(-del(ii),1) * rotmat(lam(ii),3);
        d1(:,ii) = R*northvec;
