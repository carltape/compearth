...
\item Things you should type at the prompt are prefixed by a dollar sign \$,
      and the output from commands is not:
\begin{lstlisting}
$ echo "A command you can run" | sed 's/can/have/g'
...
If successful, this command will generate several \verb+Makefile+s in \verb+src/+,
as well as \verb+shared/constants.h+ and \verb+shared/precision.h+, among others.
...
\begin{lstlisting}
$ ./configure --enable-cuda MPI_INC=$MPI_DIR/include
\end{lstlisting}
...
cluster has been set up. The following example shows the extra steps that
are required on the FISH cluster at ARSC. When running on your own cluster,
you should consult their documentation in case there are additional steps that
must be taken.
