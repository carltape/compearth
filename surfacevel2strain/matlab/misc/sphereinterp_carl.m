...
switch dopt
    ...
    case 8
        % alaska splitting - local
        qmin = 6; qmax = 8;
        nlam = 40; ilampick = 2;
        ntrsh = 3;
        nx = 20;    % number of points controlling plotting grid

        % alaska splitting - SKS
        ntrsh = 1;
        %minlampwr = -2; ilampick = 3;      % GCV local minimum
        ilampick = -12;                     % manually pick the 12th lambda value
...
if dopt==8
    ...
    figure(1); hold on; quiver(dlon,dlat,real(sqrt(Z)),imag(sqrt(Z)));
