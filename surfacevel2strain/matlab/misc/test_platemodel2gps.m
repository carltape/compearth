...
if ilon360==1, lontick = [0:60:360]; else lontick = [-180:60:180]; end
lattick = [-90:30:90];
deg = 180/pi;
earthr = 6371*1e3;
...
if q ~= 99     % specify order q grid
    ...
    if ilon360==1, ax1 = [0 360 -90 90]; else ax1 = [-180 180 -90 90]; end
    [lon,lat,~,~,~] = getspheregrid(ax1,q,q);  
    if ilon360==1, lon = wrapTo360(lon); end
...
%     ax1 = [-180 180 -90 90];
    ...
    slabel = 'alaska'; ax1 = [154 240 47 74];
    
    lonmin = ax1(1); lonmax = ax1(2);
    latmin = ax1(3); latmax = ax1(4);
    
    % coase uniform lon-lat mesh for plotting arrows
    numx = 12;
...
for irow = 4:4     % KEY: loop over fixed plates
    ...
    if and(iwrite == 1, q == 99)
        ...
        fid = fopen([odir ww3],'w');
        fprintf(fid,'%18.8e%18.8e%18.8e%18.8e\n',lonmin,lonmax,latmin,latmax); 
        fprintf(fid,'%18.8e%18.8e%18.8e%18.8e\n',min(xvec),max(xvec),min(yvec),max(yvec)); 
        fprintf(fid,'%18.8e%18.8e\n',dx_mag,dy_mag); 
        fprintf(fid,'%14i%14i\n',numx_mag,numy_mag);

        disp(['write these files in dir ' odir ' :']);
        disp(ww1); disp(ww2); disp(ww3); 
    end
