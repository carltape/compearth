...
q      = 4;         % 99 for non-global gridpoints (q = 0-7 for global gridpoints)

iwrite = 1;         % write surface v-field to file
ifig_extra = 0;     % extra figures
ipick_figs = 0;     % figures of example plate w.r.t. fixed plate

mod_labs = {'oneill','nuvel1A_nnr','revel','bird','gripp_hs3','bird_gripp','bird_morgan'};  % PLATE MODELS
...
%     ax1 = [-180 180 -90 90];
    ...
    % file label and region
    %slabel = 'japan'; ax1 = [100 160 5 60];
    %slabel = 'jdf'; ax1 = [-135 -115 35 55];
    slabel = 'micros'; ax1 = [115 160 -15 20];
    %slabel = 'socal'; ax1 = [-122 -113 30 38];
    %slabel = 'sumatra'; ax1 = [85 110 -10 30];
    %slabel = 'taiwan'; ax1 = [115 125 18 28];
    %slabel = 'tibet'; ax1 = [52 104 12 44];
    %slabel = 'eurasia'; ax1 = [-10 125 5 50];
    ...
    % denser mesh for plotting the magnitude of the velocity field
    numx_mag = 10*numx;
...
if imake==1
    ...
    switch imodel
        ...
        case 3,
            ...
            % load rotation vectors (units in deg/Myr)
            % (see test_euler_rot_tec.m)
            % MODIFIED TO MATCH THE BIRD LABELLING SCHEME
...
for irow = 4:4     % KEY: loop over fixed plates
    ...
    if and(iwrite == 1, q == 99)
        ...
        fid = fopen([dir_plates 'surface_velocities/misc/' ww2],'w');
        for ii=1:num_mag
        ...
        fid = fopen([dir_plates 'surface_velocities/misc/' ww3],'w');
        fprintf(fid,'%18.8e%18.8e%18.8e%18.8e\n',lonmin,lonmax,latmin,latmax); 
        fprintf(fid,'%18.8e%18.8e%18.8e%18.8e\n',min(xvec),max(xvec),min(yvec),max(yvec)); 
        fprintf(fid,'%18.8e%18.8e\n',dx_mag,dy_mag); 
        fprintf(fid,'%14i%14i\n',numx_mag,numy_mag);

        disp(' write to file:'); disp(ww1); disp(ww2); disp(ww3); 
    end
    ...
    if iwrite == 1
        ...
        disp('  ');
        if ifix~=99, disp([' fixed plate is ' names{ifix}]); 
        else disp(' no fixed plate: use original Euler vectors'); end
...
%         fclose(fid);
        ...
        if ipick_figs==1
            ...
            % for GMT plotting, set zero values to <0
            veps = 1e-4; izero = find( vmag_uniform < veps); vmag_uniform(izero) = -veps;
            ...
            % write plate model vector MAGNITUDES (fixed ifix plate) to
            % file (mm/yr)
