...
function rss_vec = ocv_carl_pm(d, A, lamvec, ndim, scales)
pow2scales = 2.^scales;
% Size of inputs
[ndata, nparm]  = size(A);
...
% end
% if ndim == 2
%     Dmat = diag([pow2scales.^2; pow2scales.^2]);
% end
% if ndim == 3
%     Dmat = diag([pow2scales.^2; pow2scales.^2; pow2scales.^2]);
% end
...
for ii=1:numlam
    ...
    if 1==1
        ...
        for jj=1:ndata
            ...
            f_h = inv(X'*X + lam^2*diag([pow2scales.^2; pow2scales.^2]))*X'*g;
