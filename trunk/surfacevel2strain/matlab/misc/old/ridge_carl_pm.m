...
function [m, rss, mss, Gvec, Fvec, dof, kap, iL, iGCV, iOCV] = ...
    ridge_carl_pm_old(dvec, X, hvec, ndim, scales)
...
% for j = 1:nh
%     m(:, j) = V(:, 1:q) * (zeta ./ (s2 + (hvec(j)*pow2scales).^2));
%     mss(j)  = sum(m(:, j).^2);
%     rss(j)  = (hvec(j)*pow2scales)^4 * sum(fc.^2 ./ (s2 + (hvec(j)*pow2scales).^2).^2);
%     dof(j)  = n - sum(s2./(s2 + (hvec(j)*pow2scales).^2));
% end
%
...
[iL,kap] = curvature(ones(nh,1),ones(nh,1));
%h_L = hvec(iL);
%rss_L = 10^x1(iL);
%mss_L = 10^y1(iL);
...
Fvec = ocv_carl_pm(dvec, X, hvec,ndim,scales);
[Fmin,iOCV] = min(Fvec);
