...
$idata = 11;        # choose GPS dataset (1 = NASA REASON; 2 = CCMM; 1X = strike-slip; 2X = rotational)
...
if ($k == 1 ) {
   ...
   $itag = "socal";
   $z_title = -0.15;
   $xtick1 = 1; $xtick2 = 0.25; $wid = 3;
   $origin = "-X1.0 -Y7";
   $xlegend = 0.25; $ylegend = -0.5;
...
if ($idata >= 10) {
  ...
  if ($idata >= 60 and $idata < 70) {
    $igc = 1; $ifault = 0; $iplate = 0;
    $vec_ref = 10; $vec_conf = 0.9; $vscale = 0.04;
...
$Bscale_strain  = sprintf("-B%2.2f:\"Strain rate, 10\@+%2.2i\@+ yr\@+-1\@+\": -E10p",$cmaxstrain*0.25,$cpwr3);
$Bscale_rot    = sprintf("-B%2.2f:\"Rotation rate, 10\@+%2.2i\@+ yr\@+-1\@+\": -E10p",$cmaxrot*0.25,$cpwr4);
...
if ($k == 1) {
  open(CSH,">$cshfile");
  print CSH "gmtset BASEMAP_TYPE plain MEASURE_UNIT inch PAPER_MEDIA letter PLOT_DEGREE_FORMAT D TICK_LENGTH $tick LABEL_FONT_SIZE $fsize2 ANOT_FONT_SIZE $fsize2  HEADER_FONT $fontno ANOT_FONT $fontno LABEL_FONT $fontno HEADER_FONT_SIZE $fsize1 FRAME_PEN $fpen TICK_PEN $tpen\n";
}
...
$cptvmag = "color1.cpt";
print CSH "makecpt -C$colorbar $Tvmag -D > $cptvmag \n";
