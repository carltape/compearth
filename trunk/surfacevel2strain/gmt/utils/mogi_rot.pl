...
if ($idata >= 10) {
  ...
  if ($idata >= 80 and $idata < 90) {
    ...
    $vumax = 80; $vumin = -$vumax; $vsmin = -35; $vsmax = 0; $vemin = -35; $vemax = -$vemin;
    $evec_ref = 8; $escale = 0.5;
    $fac_min = 1.0;   # 1.0 for uniform color scaling
    $fac_max = 1.0;    # 1.0 default
    if ($idata == 80) {
...
$cptup = "color_up.cpt"; print CSH "makecpt -C$colorbar $Tup -D > $cptup \n";
$cptsouth = "color_south.cpt"; print CSH "makecpt -C$colorbar $Tsouth -D > $cptsouth \n";
$cpteast = "color_east.cpt"; print CSH "makecpt -C$colorbar $Teast -D > $cpteast \n";
...
$vec_info = "$vec_info0 -A0.25p/3p/1.5p -W0.25p";
print CSH "psvelo $gpstemp $J1 $R1 $vec_info -K -V -O >> $psfile \n";
...
#}
if ($iplate==1 || $idata==1) {print CSH "psxy $J1 $R1 ${plate_file} $plate_infoW -K -O -V >> $psfile\n";}
...
print CSH "psxy -JX10 -R0/1/0/1 -W1p -K -O -V -Xa-3.6 -Ya-0.75 >>$psfile<<EOF\n0 0\n0.67 0\nEOF\n";

  #=======================

  $title = "(e)  Vertical velocity (mm/yr)";
  $ctick = ($vumax - $vumin)/4;
  $Bscale  = "-B${ctick}:\" \": -E10p";
  $B = $B0.$Bopts[7];
  $shift = $shift2;
...
if ($igc==1) {print CSH "psxy $J1 $R1 ${gc_boundary} $plate_infoW -K -O -V >> $psfile\n";}
  ...
  print CSH "pstext -N $J_title $R_title -K -O -V >>$psfile<<EOF\n $x_title $z_title2 $fsize_title 0 $fontno CM $title \nEOF\n";
  #print CSH "pstext -N $textinfo -JX1/1 -R0/1/0/1 -K -O -V >>$psfile<<EOF\n $xlab $ylab 14 0 $fontno CM $labs[1] \nEOF\n";

#================================

$shift = $shift3;
$title = "(f)  Euler vectors";

$R1 = "-R-180/180/-70/70";
$coast_infoK = "-Dl -W0.25p,0/0/0 -A1000 -S200";
$B0 = "-Ba30f10d::";
$B = $B0.$Bopts[15];
...
$x1 $ytxt[4] 11 0 $fontno CM pos     $unit_rot     neg
...
EOF\n";
  #-----------------------------
      ...
      # socal box
print CSH "psxy $J1 $R1 -W1.0p,0/0/0 -G255/255/0 -A -O -K -V <<EOF>>$psfile\n"; 
...
print CSH "pstext -N $J_title $R_title -K -O -V >>$psfile<<EOF\n $x_title 0.63 $fsize_title 0 $fontno CM $title \nEOF\n";
